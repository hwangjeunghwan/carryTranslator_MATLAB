...
%
    ...
    %set(getselectedsource(colorVid), 'TrackingMode', 'Skeleton');
    alphabet=char(65:90);
...
%                i = i+1; 
%                if (mod(i,5)==1)
%                   imwrite(handDepthImage, strcat('hangle/は/は','_',num2str(m),'.png'),'png');
%                   m=m+1;
%                end
%            end
           if ~isempty(handColorImage)
               ...
               i = i+1; 
               if (mod(i,5)==1)
                  imwrite(imresize(handColorImage,[224,224]), strcat('hanguel/た/た_',num2str(m),'.png'),'png');
                  m=m+1;
               end
           end
       end
